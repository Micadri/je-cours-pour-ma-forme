...
<template>
  <main class="p-5 font-body max-w-[600px] mx-auto text-center">
    ...
    <div class="text-left mb-5">
      <button @click="quitSession"
        class="bg-transparent border-none text-red-500 text-base font-bold cursor-pointer hover:opacity-80 transition-opacity">
        ← Quitter la course
      </button>
    </div>

    <!-- Pop-up de fin -->
    <div v-if="showEndPopup"
      class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-5 backdrop-blur-sm">
      <div
        class="bg-surface p-8 rounded-xl w-full max-w-[320px] relative shadow-2xl dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
        <button @click="closeAndSave"
          class="absolute top-3 right-4 bg-transparent border-none text-2xl text-gray-400 cursor-pointer hover:text-gray-800 dark:hover:text-gray-200">×</button>
        <h2 class="mt-0 font-heading text-2xl text-accent font-bold mb-6">Session terminée ! 🏆</h2>

        <div
          class="my-6 bg-gray-50 p-4 rounded-lg flex justify-around dark:bg-gray-900 border border-gray-100 dark:border-gray-700">
          <div>
            <strong class="text-2xl text-primary font-heading dark:text-gray-100">{{ finalStats.distance }} <span
                class="text-sm">km</span></strong>
        ...
        <div v-if="!isGuest">
          <button @click="shareSession"
            class="w-full p-4 mb-3 bg-blue-500 text-white rounded-cta font-bold cursor-pointer hover:bg-blue-600 transition-colors uppercase tracking-wider font-heading">Partager
            l'exploit</button>
          <button @click="closeAndSave"
            class="w-full p-4 bg-accent text-primary rounded-cta font-bold cursor-pointer hover:bg-opacity-90 transition-colors uppercase tracking-wider font-heading">Valider
            & Quitter</button>
        </div>
        <div v-else>
          <p class="text-gray-500 text-sm mb-5 italic dark:text-gray-400">Créez un compte pour sauvegarder cette course
            et débloquer la suite du programme !</p>
          <button @click="closeAndRegister"
            class="w-full p-4 bg-accent text-primary rounded-cta font-bold cursor-pointer hover:bg-opacity-90 transition-colors uppercase tracking-wider font-heading shadow-lg">S'inscrire</button>
        </div>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <TimerDisplay :timeRemaining="timeRemaining" :totalTime="currentExercise.duration_seconds"
        :exerciseType="currentExercise.type" />

      <div class="flex justify-between items-stretch gap-2 mt-8 w-full max-w-[400px] mx-auto">
        <button @click="prevStep" :disabled="currentStepIndex === 0"
          class="flex-1 px-1 py-4 rounded-cta font-bold text-xs sm:text-sm cursor-pointer transition-colors disabled:opacity-50 font-heading uppercase tracking-wide"
          :class="currentStepIndex === 0 ? 'bg-gray-200 text-gray-400 dark:bg-gray-700 dark:text-gray-500' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600'">
          Préc.
        </button>

        <button v-if="!isRunning" @click="startTimer"
          class="flex-[2] px-2 py-4 bg-accent text-primary rounded-cta font-bold text-base sm:text-lg cursor-pointer hover:bg-opacity-90 transition-transform hover:scale-105 shadow-md font-heading uppercase tracking-widest">
          Démarrer
        </button>
        <button v-else @click="pauseTimer"
          class="flex-[2] px-2 py-4 bg-red-500 text-white rounded-cta font-bold text-base sm:text-lg cursor-pointer hover:bg-red-600 transition-colors shadow-md font-heading uppercase tracking-widest animate-pulse">
          Pause
        </button>

        <button @click="nextStep"
          class="flex-1 px-1 py-4 bg-gray-200 text-gray-700 rounded-cta font-bold text-xs sm:text-sm cursor-pointer hover:bg-gray-300 transition-colors dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 font-heading uppercase tracking-wide">
          Suiv.
        </button>
      </div>
    </div>
