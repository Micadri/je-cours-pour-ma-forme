...
<template>
  <main style="padding: 20px; font-family: sans-serif; max-width: 900px; margin: 0 auto; position: relative;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
      ...
      <button @click="handleLogout" style="padding: 8px 15px; background: #f44336; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: bold;">Déconnexion</button>
    </div>

    <div v-if="errorMessage" style="background: #ffebee; color: #d32f2f; padding: 15px; border-radius: 8px; text-align: center; font-weight: bold; margin-bottom: 20px;">{{ errorMessage }}</div>

    <div style="display: flex; gap: 10px; margin-bottom: 20px; border-bottom: 2px solid #eee; padding-bottom: 10px; flex-wrap: wrap;">
      <button @click="activeTab = 'runners'" :style="{ background: activeTab === 'runners' ? '#4CAF50' : 'transparent', color: activeTab === 'runners' ? 'white' : '#666', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }">👥 Les Coureurs</button>
      <button @click="activeTab = 'feedbacks'" :style="{ background: activeTab === 'feedbacks' ? '#4CAF50' : 'transparent', color: activeTab === 'feedbacks' ? 'white' : '#666', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }">📬 Signalements</button>
      <button @click="activeTab = 'program'" :style="{ background: activeTab === 'program' ? '#4CAF50' : 'transparent', color: activeTab === 'program' ? 'white' : '#666', border: 'none', padding: '10px 20px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }">📁 Gérer le Programme</button>
    </div>
    ...
    <AdminFeedbacks v-if="activeTab === 'feedbacks'" :feedbacks="feedbacks" :isLoading="isFeedbacksLoading" @refresh="fetchFeedbacks" @delete="deleteFeedback" />
